...
const mockResolver =
  (config: any): any =>
  (path?: string) =>
    path ? config[path as keyof typeof config] : config;
...
describe("configHandler", () => {
  describe("options", () => {
    ...
    test("should return default config", () => {
      ...
      expect(resolved).toEqual(iconComponent);
    });
  });
  ...
  describe("override", () => {
    test("Should return the config", () => {
      ...
      expect(
        configResolver("icon", config as Config, { rootPath: "components" })
      ).toEqual(iconComponent);
    });
  });
