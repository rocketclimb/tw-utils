import { Config, ResolverOptions } from "@/types";
...
export const configResolver = <T>(
  ...
  const extendRequestedCfg =
    extendCfg &&
    ((options?.rootPath && extendCfg[options?.rootPath]) || extendCfg)[path];
  return extendRequestedCfg
    ? deepMerge({ ...requestedCfg }, { ...extendRequestedCfg })
    : { ...requestedCfg };
};
